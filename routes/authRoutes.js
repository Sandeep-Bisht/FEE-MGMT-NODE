...
router.post('/StoreStudent', upload.fields([{
    ...
    router.put('/updateStudent', upload.fields([{
        ...
        Student.findByIdAndUpdate({_id:old_id},{image,image2,image3,image4,unique_id,school_id,session,date_of_admission,parent,admission_no,security_no,old_admission_no,aadhar_no,class_name,section,subjects,is_start_from_first_class,last_class:oldclass_name,category,house,name,sex,dob,nationality,reg_no,roll_no,board_roll_no,last_school,balance,fee_concession,bus_fare_concession,vehicle_no,is_teacher_ward,paid_upto_month,paid_upto_year,last_school_performance,is_full_free_ship,avail_transport,take_computer,no_exempt_security_deposit,ncc,no_exempt_registration,no_exempt_admission,is_repeater,other_details,misc_details,account_no,father_name,mother_name,father_occu,father_designation,father_annual_income,mother_occu,mother_desgination,mother_annual_income,parent_address,parent_city,parent_state,parent_country, parent_per_address,parent_per_city,parent_per_state,parent_per_country,parent_phone,parent_mobile,gaurdian_name,gaurdian_occu,gaurdian_designation,gaurdian_annual_income,gaurdian_address,gaurdian_city,gaurdian_state,gaurdian_country,gaurdian_per_address,gaurdian_per_city,gaurdian_per_state,gaurdian_per_country,gaurdian_phone,gaurdian_mobile,religion }, function(err, result){
            if(err){                
                res.send(err)
            }
            else{
...
router.post('/StoreReceipt', upload.single('image'), async (req, res) => {
    ...
    try {
        ...
        let receipt_no;
        if (class_name == "KG" || class_name == "PG" || class_name == "NURSERY" || class_name == "I" || class_name == "II" || class_name == "III" || class_name == "IV" || class_name == "V") {
...
router.post('/StoreCSVentry', upload.single('image'), async (req, res) => {
    ...
    // start dropstatus Api
    router.post('/DropStatus', upload.single('image'), async (req, res) => {
